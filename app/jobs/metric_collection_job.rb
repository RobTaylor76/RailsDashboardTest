class MetricCollectionJob < ApplicationJob
  ...
  def perform
    ...
    Metric.create_metric('cpu', cpu_usage, '%')
    Metric.create_metric('memory', memory_usage, '%')
    Metric.create_metric('disk', disk_usage, '%')
    Metric.create_metric('network', network_speed, 'MB/s')
